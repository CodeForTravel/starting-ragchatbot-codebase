...
def test_live_content_query(tmp_path):
    ...
    cfg = type(config)(GEMINI_API_KEY=config.GEMINI_API_KEY, CHROMA_PATH=str(tmp_path / "c"))
    rag = RAGSystem(cfg)
    rag.add_course_folder(docs)
    answer, sources = rag.query("What is covered in lesson 0 of the computer use course?")
...
def test_live_outline_then_unscoped_search(tmp_path):
    ...
    answer, _ = rag.query("Find another course that covers the same topic as lesson 4 of the MCP course.")
